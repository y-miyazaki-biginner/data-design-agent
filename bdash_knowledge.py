...
USE_CASE_PATTERNS = [
    {
        "id": "cart_reminder",
        "name": "当日カゴ落ちリマインド",
        "description": "ECサイトでカートに入れたが購入していない顧客に当日リマインドメールを送る（b→dashサポートページ準拠）",
        "data_sources": ["顧客データ", "注文データ", "注文明細データ", "商品データ", "Webアクセスログ", "Webコンバージョンデータ"],
        "steps": [
            "❶ カート投入取得データの構築: Webアクセスログからセッション×顧客ID単位で集約した顧客情報と、セッション×商品ID単位で集約したカート投入日時を横統合",
            "❷ カートページ取得データの構築: Webコンバージョンデータからカートページ閲覧の商品IDを取得し、顧客別にカート内の上位3商品を抽出（ランキング処理）",
            "❸ カートデータ全量: ❶と❷を縦統合し、全カート行動を一元化",
            "❹ 購入実績マスタ: 注文データ×注文明細データ×商品データを横統合し、顧客ID×商品ID単位で最終受注日時を集約",
            "❺ 未購入かご落ち商品リスト: ❸と❹を外結合し、絞り込み（カート投入から1日以内かつ未購入）で抽出。商品情報を連結・分割処理で差し込み用に整形",
            "❻ 配信用最終データ: 顧客データと❺を横統合（customer_id）し、メールアドレス・かご落ち経過日数を付与",
            "セグメント → コンテンツ（商品画像・URL差し込み） → メール配信",
        ],
        "key_operations": ["集約", "横統合", "縦統合", "絞り込み", "連結", "分割", "IF文", "時刻演算", "ランキング"],
        "required_setup": "サイトのアクセスログ計測とリレーション設定が必須（顧客ID、カート投入商品ID、カートページ閲覧商品IDの取得設定）",
        "tips": [
            "期間条件: 開始日「1日前」、終了日「0日前」で当日かご落ちを抽出",
            "ランキング処理: カート投入順を付番し上位3商品に絞込み",
            "連結・分割: 複数商品情報を「|」区切りで連結後、商品ごとに分割して差し込み形式に整形",
            "IF分岐: 差し込み商品数を判定し配信テンプレートを制御",
        ],
    },
    {
